...
A question can carry one media: an image, a video or a sound (`media.type` is `"image"`, `"video"` or `"audio"`). The projected screen loads it itself from its address, `media.url`, and so does every phone for an image (a video or a sound plays on the projected screen only, see below). The address must be one of:

- a **direct link to the file**, a web address starting with `http://` or `https://` (`https://intranet.example/films/consignes.mp4`), that the devices showing it can reach (the projected screen's computer for a video or a sound, every phone too for an image): a file on the intranet is only seen on the intranet, and a file behind a sign-in page is not seen on the phones;
...
In the editor, a line under the type says where a video or a sound plays (« La vidéo passe sur l'écran projeté… »). Where it plays is kept in `media.playback`: `"screen"`, the projected screen, is the default and the only way it plays in this version (`"devices"` is kept for a later one). A stored quiz whose `playback` is a value the game does not know is read as the screen, never refused; a save refuses it.
