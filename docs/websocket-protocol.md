...
| `SHOW_PREPARED` | `{ totalAnswers: number, questionNumber: number, questionType: "single" \| "multi" \| "truefalse" \| "poll" \| "slide" }`                                                         | Short transition before a question is shown: its number, its type, and how many answer options it has.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `SHOW_QUESTION` | `{ question: string, media?, upcomingMedia?: "video" \| "audio", cooldown: number, answers: string[], questionType, time: number, totalPlayer: number }`                          | Reading time: the question and its answers are shown, but answers are **not** accepted yet (the server ignores a submission sent now). `cooldown` is how many seconds until answers open. `media` is only an image at this step; `upcomingMedia` announces a video or audio that starts with `SELECT_ANSWER`. The solutions are never sent to players.                                                                                                                                                                                                                                                            |
| `SELECT_ANSWER` | `{ question, answers: string[], media?, time: number, totalPlayer: number, questionType: "single" \| "multi" \| "truefalse" \| "poll" \| "slide", options? }`                     | Answers are open. `answers.length` tells you how many buttons are relevant (2-4, none for a slide). `time` is the number of seconds to answer. `questionType` is `"single"` (one correct button), `"multi"` (one or more), `"truefalse"` (two buttons, one correct), `"poll"` (a vote, no correct button) or `"slide"` (an info screen, nothing to submit, `answers` is empty).                                                                                                                                                                                                                                   |
| `SHOW_RESULT`   | `{ outcome: "correct" \| "wrong" \| "noAnswer" \| "voted" \| "noVote", correct: boolean, message: string, points: number, myPoints: number, rank: number, totalPlayers: number }` | How the question ended for you. `outcome` is `"correct"`, `"wrong"` or `"noAnswer"` on a scored question, `"voted"` or `"noVote"` on a `"poll"`. `correct` keeps its meaning: whether your answer was right, or on a `"poll"` only that your vote was recorded. `message` is the matching i18n key: `game:correct`, `game:wrong`, `game:noAnswer`, `game:pollAnswered` or `game:pollNoVote`. `points` is the change actually applied to your total this question (negative for a penalty, after the floor at 0), `myPoints` your new total, `rank` your position out of `totalPlayers`. Not sent for a `"slide"`. |
| `WAIT`          | `{ text: string }`                                                                                                                                                                | Generic waiting screen (e.g. after answering, waiting for other players or for the manager to continue).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `FINISHED`      | `{ subject: string, top: Player[], rank?: number, totalPlayers?: number }`                                                                                                        | Game over: the top 5 of the final leaderboard, and for a player their final `rank` out of `totalPlayers`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- Points are time-weighted (faster correct answers score higher), computed server-side from `time` and when you answer relative to the start of the answer window. When `time` is `-1` (no limit), points depend on answer order instead.
