...
| `SHOW_PREPARED` | `{ totalAnswers: number, questionNumber: number, questionType: "single" \| "multi" \| "truefalse" \| "poll" \| "slide" \| "ordering" \| "shortanswer" }`                                       | Short transition before a question is shown: its number, its type, and how many answer options it has (the length of the `answers` list to come: 0 for a `"shortanswer"`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `SHOW_QUESTION` | `{ question: string, media?, upcomingMedia?: "video" \| "audio", cooldown: number, answers: string[], questionType, time: number, totalPlayer: number }`                                       | Reading time: the question and its answers are shown, but answers are **not** accepted yet (the server ignores a submission sent now). `cooldown` is how many seconds until answers open. `media` is only an image at this step; `upcomingMedia` announces a video or audio that starts with `SELECT_ANSWER`. `answers` is the same list as in `SELECT_ANSWER`: shuffled for an `"ordering"`, empty for a `"shortanswer"`. The solutions, the accepted answers of a `"shortanswer"` and the correct order of an `"ordering"` are never sent to players.                                                                                                                                                                                                                                                                                                 |
| `SELECT_ANSWER` | `{ question, answers: string[], media?, time: number, totalPlayer: number, questionType: "single" \| "multi" \| "truefalse" \| "poll" \| "slide" \| "ordering" \| "shortanswer", options? }`   | Answers are open. `answers.length` tells you how many buttons are relevant (2-4, none for a slide). `time` is the number of seconds to answer. `questionType` is `"single"` (one correct button), `"multi"` (one or more), `"truefalse"` (two buttons, one correct), `"poll"` (a vote, no correct button), `"slide"` (an info screen, nothing to submit, `answers` is empty), `"ordering"` (3 to 6 items to put in order, `answers` is the shuffled list, the same for every player and after a reconnection) or `"shortanswer"` (a text to type, `answers` is empty).                                                                                                                                                                                                                                                                                  |
| `SHOW_RESULT`   | `{ outcome: "correct" \| "partial" \| "wrong" \| "noAnswer" \| "voted" \| "noVote", correct: boolean, message: string, points: number, myPoints: number, rank: number, totalPlayers: number }` | How the question ended for you. `outcome` is `"correct"`, `"wrong"` or `"noAnswer"` on a scored question, `"voted"` or `"noVote"` on a `"poll"`; an `"ordering"` can also end `"partial"` (some items at their place, not all). `correct` keeps its meaning: whether your answer earned credit (true for `"partial"` too), or on a `"poll"` only that your vote was recorded. `message` is the matching i18n key: `game:correct`, `game:partial`, `game:wrong`, `game:noAnswer`, `game:pollAnswered` or `game:pollNoVote`. `points` is the change actually applied to your total this question (negative for a penalty, after the floor at 0), `myPoints` your new total, `rank` your position out of `totalPlayers`. On a `"partial"` ordering only, `placed: { count, total }` gives how many items you put at their place. Not sent for a `"slide"`. |
...
`data` holds either `answerKeys` or `text`, never both; anything else is dropped.

- `answerKeys` are 0-based indices into the `answers` array received in `SELECT_ANSWER`. For a `"single"` question, send a one-element array, e.g. `[1]` for the second button. For `"multi"`, send every button pressed, e.g. `[0, 2]`. For `"poll"`, send the single button voted for: no answer is correct, so no points are awarded. A `"slide"` takes no submission at all.
...
| `player:selectedAnswer` | `{ gameId, data: { answerKeys: number[] } \| { text: string } }` |
...
Once a question closes, only the manager receives `SHOW_RESPONSES`: the question itself (with `solutions`, and `accepted` for a `"shortanswer"`) plus these fields.

| Field           | Meaning                                                                                                                                                                                                                         |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `responses`     | Counts keyed by index. Choice types: votes per answer. `"ordering"`: players who put item `i` at its place, `i` indexing `answers`, which is in the correct order here. `"shortanswer"`: inputs recognized per accepted answer. |
...
| `partialCount`  | Scored types only: answers with some credit, not all (an `"ordering"` partly right, a `"multi"` in a partial mode).                                                                                                             |
...
On a `"shortanswer"`, the inputs that matched no accepted answer are `totalAnswered - correctCount`; `SHOW_RESPONSES` never carries the texts players typed.
