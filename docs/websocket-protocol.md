...
A question's video or sound plays on the projected screen only. The manager's own copy of `SHOW_QUESTION` and `SELECT_ANSWER` carries the whole `media` (`{ type, url, playback? }`), where the players' copy carries its type alone; a manager that reconnects gets that copy again, and `SHOW_RESPONSES` keeps the whole media. The web screen loads it during the reading time, starts it with `SELECT_ANSWER` (with `SHOW_QUESTION` on a `"slide"`), and keeps the same player, playing on or paused, through `SHOW_RESPONSES`, until the next status. Play, pause, back to the start and full screen stay on the manager's screen: no event reaches the server in this version.
