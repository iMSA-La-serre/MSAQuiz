...
- Points are time-weighted (faster correct answers score higher), computed server-side from `time` and when you answer relative to the start of the answer window. When `time` is `-1` (no limit), points depend on answer order instead. A question can switch this speed bonus off (`speedBonus`, off by default for `"ordering"`, `"shortanswer"`, `"estimate"`, `"highlight"`, `"statements"`, `"categorize"`, `"ranking"` and `"scale"`): every right answer then earns the full points.
