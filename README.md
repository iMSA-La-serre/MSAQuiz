...
The JSON format, the fifteen question types (`single`, `multi`, `truefalse`, `poll`, `slide`, `ordering`, `shortanswer`, `wordcloud`, `estimate`, `highlight`, `statements`, `categorize`, `ranking`, `scale`, `markers`), the scoring modes and the spreadsheet import rules are documented in **[docs/quiz.md](docs/quiz.md)**.
