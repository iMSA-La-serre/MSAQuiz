...
// The passages a player tapped, each ticked when it was one to spot, and the
// share of the points the answer earned.
const HighlightResultCells = ({ question, record }: ResultCellsProps) => {
  const { t, i18n } = useTranslation()
  const { answerIds } = record
  const score = highlightScore(question, record)
              ...
              .map((id) => (
                ...
                >
                  ...
                  {question.solutions.includes(id) && (
                    <Check
                      aria-label={t("manager:result.toSpot")}
...
export default HighlightResultCells
